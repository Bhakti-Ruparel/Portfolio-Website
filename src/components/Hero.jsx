import './Hero.css'
import profilePhoto from '../assets/hero.png'

export default function Hero() {
  return (
    <section className="hero" id="hero" aria-label="Introduction">
      <div className="hero-bg" aria-hidden="true" />
      <div className="hero-glow" aria-hidden="true" />
      <div className="hero-glow-2" aria-hidden="true" />

      <div className="hero-inner">
        <div className="hero-content">
          <div className="hero-badge" role="status" aria-label="Open to opportunities">
            <span className="badge-dot" aria-hidden="true" />
            <span className="badge-text">Open to Opportunities</span>
          </div>

          <h1>
            Hi, I'm{' '}
            <span className="hero-gradient-text">Ruparel Bhakti</span>
          </h1>

          <p className="hero-subtitle">
            <strong>AI / Full-Stack Developer</strong> from Vadodara, India.
            I build scalable AI and web applications with FastAPI, React.js, and data-driven engineering —
            spanning AI product development, backend APIs, and modern frontend experiences.
          </p>

          <div className="hero-actions">
            <a href="#projects" className="btn-hero-primary" aria-label="View my projects">
              View Projects
              <span aria-hidden="true">→</span>
            </a>
            <a href="#contact" className="btn-hero-secondary" aria-label="Contact me">
              Contact Me
            </a>
          </div>

          <div className="hero-stats" role="list" aria-label="Key highlights">
            <div className="stat-item" role="listitem">
              <span className="stat-value">B.Tech</span>
              <span className="stat-label">AI &amp; ML</span>
            </div>
            <div className="stat-item" role="listitem">
              <span className="stat-value">AI/ML</span>
              <span className="stat-label">Intern</span>
            </div>
            <div className="stat-item" role="listitem">
              <span className="stat-value">FastAPI</span>
              <span className="stat-label">+ React</span>
            </div>
          </div>

          <div className="hero-social" role="list" aria-label="Social links">
            <a
              href="https://github.com/Bhakti-Ruparel"
              target="_blank"
              rel="noreferrer"
              className="social-link"
              role="listitem"
              aria-label="GitHub profile"
            >
              <svg className="social-icon" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                <path d="M12 0C5.37 0 0 5.37 0 12c0 5.31 3.435 9.795 8.205 11.385.6.105.825-.255.825-.57 0-.285-.015-1.23-.015-2.235-3.015.555-3.795-.735-4.035-1.41-.135-.345-.72-1.41-1.23-1.695-.42-.225-1.02-.78-.015-.795.945-.015 1.62.87 1.845 1.23 1.08 1.815 2.805 1.305 3.495.99.105-.78.42-1.305.765-1.605-2.67-.3-5.46-1.335-5.46-5.925 0-1.305.465-2.385 1.23-3.225-.12-.3-.54-1.53.12-3.18 0 0 1.005-.315 3.3 1.23.96-.27 1.98-.405 3-.405s2.04.135 3 .405c2.295-1.56 3.3-1.23 3.3-1.23.66 1.65.24 2.88.12 3.18.765.84 1.23 1.905 1.23 3.225 0 4.605-2.805 5.625-5.475 5.925.435.375.81 1.095.81 2.22 0 1.605-.015 2.895-.015 3.3 0 .315.225.69.825.57A12.02 12.02 0 0 0 24 12c0-6.63-5.37-12-12-12z"/>
              </svg>
              GitHub
            </a>
            <a
              href="https://linkedin.com/in/bhakti-ruparel"
              target="_blank"
              rel="noreferrer"
              className="social-link"
              role="listitem"
              aria-label="LinkedIn profile"
            >
              <svg className="social-icon" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 0 1-2.063-2.065 2.064 2.064 0 1 1 2.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
              </svg>
              LinkedIn
            </a>
          </div>
        </div>

        <div className="hero-avatar-wrap" aria-hidden="true">
          <div className="hero-avatar">
            <img src={profilePhoto} alt="Ruparel Bhakti" className="avatar-photo" />
          </div>
          <div className="avatar-ring" />
        </div>
      </div>

      <div className="scroll-cue" aria-hidden="true">
        <span>Scroll</span>
        <span className="scroll-arrow">↓</span>
      </div>
    </section>
  )
}
